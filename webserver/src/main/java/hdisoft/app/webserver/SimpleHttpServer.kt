...
class SimpleHttpServer(
    ...
    companion object {
        ...
        fun sendHttpResponse(output: OutputStream, status: String, contentType: String, content: ByteArray) {
            val headers = "$status\r\n" +
    ...
    private fun handleClient(socket: Socket) {
        ...
        try {
            ...
            // 4.5 Custom Stream Request Handler Hook (e.g. streaming large APK uploads directly to file)
            if (requestHandler?.handleStreamRequest(method, path, queryMap, contentLength, reader, output) == true) {
                return
            }

            // 5. Read HTTP Body into memory for standard requests if Content-Length > 0
            var body = ""
            if (contentLength > 0) {
            ...
            // 5.5 Custom App Request Handler Hook
            if (requestHandler?.handleRequest(method, path, queryMap, body, output) == true) {
                return
            }
            ...
            when (method) {
                "GET" -> {
                    when {
                        ...
                        path == "/logcat.html" || path == "/logcat" -> {
                            serveAssetFile(output, "logcat.html", "text/html")
                        }
                        else -> {
                ...
                "POST" -> {
                    if (path == "/api/records") {
                        ...
                        try {
                            ...
                            val launched = hdisoft.app.webserver.internal.AppLauncher.openApp(context, query)
                            if (launched) {
