...
object AppTool {
    ...
    /**
     * Get a list of user-installed (downloaded) apps on the device,
     * excluding system applications.
     */
    fun getDownloadedApps(context: Context): List<InstalledApp> {
        val pm = context.packageManager
            ...
            for (pkgInfo in installedPackages) {
                ...
                if (!isSystemApp) {
                    ...
                    if (!processedPackages.contains(pkg)) {
                        ...
                        val label = appInfo.loadLabel(pm).toString().ifBlank { pkg }
                        val verName = pkgInfo.versionName ?: ""
                        appList.add(InstalledApp(label, pkg, verName, isSystem = false))
                    }
                }
            }
        } catch (_: Exception) {}
            ...
            try {
                ...
                if (appInfo != null) {
                    val isSystemApp = (appInfo.flags and android.content.pm.ApplicationInfo.FLAG_SYSTEM) != 0 ||
                            (appInfo.flags and android.content.pm.ApplicationInfo.FLAG_UPDATED_SYSTEM_APP) != 0
                    if (!isSystemApp) {
                        val label = resolveInfo.loadLabel(pm).toString().ifBlank { pkg }
                        val verName = pkgInfo.versionName ?: ""
                        appList.add(InstalledApp(label, pkg, verName, isSystem = false))
                    }
                }
        ...
        return appList.sortedBy { it.appName.lowercase(Locale.ROOT) }
    }
}
